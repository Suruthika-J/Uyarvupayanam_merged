...
import {
  ...
  FiBookOpen, FiCpu, FiUsers, FiHelpCircle,
} from 'react-icons/fi'

// Grouped sidebar nav. AI tools live under one "AI Assistant" section so the
// product reads as a single coherent AI assistant rather than scattered pages.
  ...
  return (
    <div className="student-root" style={{ minHeight: '100vh', display: 'flex', background: '#f1f5f9' }}>
      ...
      <aside style={{
        ...
        {/* Nav Items */}
        <nav style={{ flex: 1, padding: '12px 12px 18px', display: 'flex', flexDirection: 'column', gap: 14, overflowY: 'auto' }}>
          {SIDEBAR_SECTIONS.map((section) => (
            <div key={section.title}>
              {section.title && (
                <div style={{
                  display: 'flex', alignItems: 'center', gap: 6,
                  padding: '0 14px 6px', marginTop: 2,
                }}>
                  {section.ai ? (
                    <span style={{
                      display: 'inline-flex', alignItems: 'center', gap: 5,
                      background: '#4f46e5', color: '#fff', fontSize: 9,
                  ...
                  return (
                    <Link
                      ...
                      style={{
                        ...
                        background: isActive ? '#0284c7' : section.ai ? 'rgba(79, 70, 229, 0.10)' : 'transparent',
                    ...
                    >
                      ...
                      {label}
                      {section.ai && !isActive && (
                        <span style={{ marginLeft: 'auto', fontSize: 8, fontWeight: 800, color: '#6366f1', letterSpacing: '0.05em' }}>AI</span>
                      )}
                    </Link>
                  )
                })}
