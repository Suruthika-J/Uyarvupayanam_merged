...
import { getInsightStyle, getInsightShortLabel } from '../../../constants/collegesInsightTheme'
...
export default function CollegesInsight() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      {rows.map((row) => {
        const style = getInsightStyle(row.category)
        const shortLabel = getInsightShortLabel(row.category, row.label)
        return (
          <div
            key={row.category}
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 24 }}>
              <div style={{
                ...
                boxShadow: `0 10px 20px -5px ${style.color}66`,
              }}>
                {style.icon}
              </div>
              <div>
                <h3 style={{ margin: 0, fontSize: 24, fontWeight: 900, color: '#1e293b', letterSpacing: '-0.02em' }}>{shortLabel}</h3>
            ...
            <div style={{ background: style.bg, width: 48, height: 48, borderRadius: 12, display: 'grid', placeItems: 'center', color: style.color, flexShrink: 0 }}>
