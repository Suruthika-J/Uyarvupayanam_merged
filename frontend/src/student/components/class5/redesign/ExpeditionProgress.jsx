...
export default function ExpeditionProgress({ expedition, onContribute, contributedToast }) {
  ...
  const pct = Math.max(0, Math.min(100, Math.round(Number(expedition.pct ?? (expedition.currentXp / expedition.targetXp) * 100) || 0)))
  const color = worldColor('blue')
  const goalIcon = React.createElement(worldIcon(expedition.goalWorldKey) || ICONS.flag, { size: 26, strokeWidth: 2.2 })
  return (
    <div
      style={{
        background: C5.bg,
    ...
    >
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          <div
            ...
            aria-hidden="true"
          >
            {expedition.goalImage ? (
              <span
                style={{
                  width: '100%',
                  height: '100%',
                  backgroundImage: `url(${expedition.goalImage})`,
      ...
      <div style={{ display: 'flex', gap: 10, marginTop: 18, alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        {contributedToast && (
          <span style={{ fontSize: 12.5, fontWeight: 800, color: '#059669', background: '#ecfdf5', borderRadius: 99, padding: '6px 12px', display: 'inline-flex', alignItems: 'center', gap: 5 }}>
            <ICONS.check size={13} strokeWidth={2.6} /> XP added! Thank you!
          </span>
        )}
      </div>
      ...
      {expedition.recentCheer?.length > 0 && (
        <div style={{ marginTop: 18, borderTop: `1px solid ${C5.line}`, paddingTop: 14 }}>
          <div style={{ fontSize: 12, fontWeight: 800, color: C5.muted, marginBottom: 10, display: 'flex', alignItems: 'center', gap: 6 }}>
            <ICONS.messageCircle size={13} strokeWidth={2.3} color={C5.navy} /> Recent cheers from the class
          </div>
