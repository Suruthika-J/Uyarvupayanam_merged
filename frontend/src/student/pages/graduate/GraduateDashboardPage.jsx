...
export default function GraduateDashboardPage() {
  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '60vh' }}>
        <SLoader label="Loading Graduate Dashboard..." />
      </div>
    )
  }

  const profile = data?.profile || {}
  const topCareer = data?.topCareer
  const readinessScore = data?.careerReadinessScore || 60
  ...
  return (
    <div className="s-anim-up" style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{
        ...
        <div>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(59,130,246,0.2)', color: '#60a5fa', padding: '4px 12px', borderRadius: 20, fontSize: 12, fontWeight: 800, marginBottom: 12 }}>
            <FiBriefcase size={13} /> GRADUATE CAREER PLATFORM
          </div>
          <h1 style={{ fontSize: 'clamp(1.6rem, 2.5vw, 2.2rem)', fontWeight: 900, margin: '0 0 8px', color: '#fff' }}>
            Welcome back, {student?.name || 'Graduate'}!
          </h1>
          <p style={{ color: '#94a3b8', fontSize: 14, margin: 0, maxWidth: 540 }}>
            {profile.degree ? `${profile.degree} in ${profile.domain}` : 'Degree Completed'} • Focused on <strong>{profile.primaryCareerDirection || 'Professional Growth'}</strong>
          </p>
        </div>
      ...
      {/* ── 2. METRICS ROW: READINESS SCORE & CAREER DIRECTION ── */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 20 }}>
        ...
        <SCard style={{ borderRadius: 20, padding: 26, border: '1px solid var(--s-border)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginBottom: 16 }}>
            ...
            <div>
              <div style={{ fontWeight: 800, fontSize: 16, color: 'var(--s-text)', marginBottom: 4 }}>
                {readinessScore >= 75 ? 'Strong Placement Readiness' : readinessScore >= 55 ? 'Developing Candidate' : 'Foundation Stage'}
              </div>
              <p style={{ fontSize: 13, color: 'var(--s-text3)', margin: 0, lineHeight: 1.4 }}>
                Based on education, technical skills, portfolio evidence, and interview preparation.
              </p>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, paddingTop: 14, borderTop: '1px solid var(--s-border)' }}>
            <div style={{ fontSize: 12, color: 'var(--s-text2)' }}>
              Skills Added: <strong>{(profile.technicalSkills || []).length} skills</strong>
            </div>
            <div style={{ fontSize: 12, color: 'var(--s-text2)' }}>
              Projects: <strong>{(profile.projects || []).length} logged</strong>
            </div>
          </div>
        </SCard>
        ...
        <SCard style={{ borderRadius: 20, padding: 26, border: '1px solid var(--s-border)' }}>
          ...
          <h3 style={{ fontSize: 20, fontWeight: 900, color: 'var(--s-text)', margin: '0 0 6px' }}>
            {topCareer?.title || 'Software & Technology Careers'}
          </h3>
          <p style={{ fontSize: 13, color: 'var(--s-text3)', marginBottom: 16, lineHeight: 1.5 }}>
            {topCareer?.shortDescription || 'Aligns with your degree qualifications and technical interests.'}
          </p>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: 14, borderTop: '1px solid var(--s-border)' }}>
            <span style={{ fontSize: 12, color: 'var(--s-text3)' }}>
              Growth Outlook: <strong style={{ color: '#047857' }}>{topCareer?.growthOutlook || 'High Demand'}</strong>
            </span>
            <Link to="/graduate/careers" style={{ fontSize: 13, fontWeight: 800, color: '#2563eb', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 4 }}>
              Explore Options <FiArrowRight size={14} />
            </Link>
          </div>
        </SCard>
      </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, flexWrap: 'wrap', gap: 10 }}>
          <div>
            ...
            <p style={{ fontSize: 13, color: 'var(--s-text3)', margin: 0 }}>
              What you already know vs what hiring teams require for {topCareer?.title || 'your target career'}.
            </p>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 16 }}>
          ...
          <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 16, padding: 16 }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              {data?.skillGapSummary?.strongSkills?.length > 0 ? (
                data.skillGapSummary.strongSkills.map((s, idx) => (
                  <span key={idx} style={{ background: '#dcfce7', color: '#14532d', padding: '4px 10px', borderRadius: 10, fontSize: 12, fontWeight: 700 }}>
                    {s}
                  </span>
                ))
              ) : (
                <span style={{ fontSize: 12, color: '#15803d' }}>Foundation established</span>
              )}
            </div>
          </div>
          ...
          <div style={{ background: '#fffbeb', border: '1px solid #fde68a', borderRadius: 16, padding: 16 }}>
            <div style={{ fontSize: 12, fontWeight: 800, color: '#92400e', marginBottom: 10 }}>
              🎯 Missing Critical Skills to Learn
            </div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              {data?.skillGapSummary?.missingCriticalSkills?.length > 0 ? (
                data.skillGapSummary.missingCriticalSkills.map((s, idx) => (
                  <span key={idx} style={{ background: '#fef3c7', color: '#78350f', padding: '4px 10px', borderRadius: 10, fontSize: 12, fontWeight: 700 }}>
                    • {s}
                  </span>
                ))
              ) : (
                <span style={{ fontSize: 12, color: '#b45309' }}>Reviewing syllabus competencies</span>
              )}
            </div>
          </div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 20 }}>
        ...
        {/* Competitive Exams (Only if selected) */}
        {data?.examTracker?.active && (
          <SCard style={{ borderRadius: 20, padding: 24, border: '1px solid var(--s-border)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <FiBook style={{ color: '#2563eb' }} />
                <span style={{ fontWeight: 800, fontSize: 15, color: 'var(--s-text)' }}>Competitive Exams Hub</span>
              </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {data.examTracker.exams?.map((ex, idx) => (
                <div key={idx} style={{ background: '#f8fafc', padding: 12, borderRadius: 12, border: '1px solid var(--s-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontWeight: 700, fontSize: 13, color: 'var(--s-text)' }}>{ex.examName}</span>
                  <span style={{ fontSize: 11, background: '#e0e7ff', color: '#3730a3', padding: '2px 8px', borderRadius: 6, fontWeight: 700 }}>
                    Target {ex.targetYear || '2026'}
                  </span>
                </div>
              ))}
            </div>
        ...
        {/* Higher Studies Module (Only if selected) */}
        {data?.higherStudiesTracker?.active && (
          <SCard style={{ borderRadius: 20, padding: 24, border: '1px solid var(--s-border)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <FiAward style={{ color: '#047857' }} />
                <span style={{ fontWeight: 800, fontSize: 15, color: 'var(--s-text)' }}>Postgraduate Pathways</span>
              </div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {data.higherStudiesTracker.preferredDegrees?.map((deg, idx) => (
                <span key={idx} style={{ background: '#d1fae5', color: '#065f46', padding: '6px 12px', borderRadius: 10, fontSize: 12, fontWeight: 700 }}>
                  🎓 {deg}
                </span>
              ))}
            </div>
      ...
      <SCard style={{ borderRadius: 20, padding: 26, border: '1px solid var(--s-border)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <h3 style={{ fontSize: 17, fontWeight: 800, margin: 0, color: 'var(--s-text)' }}>
            📋 This Week's Priority Action Plan
          </h3>
          <span style={{ fontSize: 12, color: 'var(--s-text3)' }}>Stay accountable to your timeline</span>
        </div>

        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {data?.weeklyPlan?.map((item, idx) => {
