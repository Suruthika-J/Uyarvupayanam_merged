...
export default function StudyPlannerPage() {
  ...
  const handleGeneratePlan = async () => {
    ...
    try {
      ...
      if (res.data?.success && res.data.plan) {
        ...
        setError('The AI couldn’t generate a plan for those inputs. Adjust your details and try again.')
      }
    } catch (err) {
      console.warn('Generate study plan failed:', err)
      setError('We couldn’t reach the study-planning engine. Please try again in a moment.')
    } finally {
      setLoading(false)
    }
  }
    ...
    if (newStatus === 'completed') {
      ...
      try {
        await axiosInstance.post(
          '/study-tools/planner/complete-task',
  ...
  return (
    <div style={{ maxWidth: 1080, margin: '0 auto' }} className="s-anim-up">
      ...
      {loading ? (
        <AIGenerating
          label="Generating your weekly study plan"
          sub="Allocating time slots across your week, prioritizing upcoming exams and weak subjects..."
        />
      ) : error ? (
        <AIFailure
          title="Couldn’t generate your study plan"
          message={error}
        ...
        <SEmpty
          ...
          desc="Click 'Generate My Weekly Plan' above to create a personalized, time-slotted study schedule."
        />
      )}
    </div>
  )
