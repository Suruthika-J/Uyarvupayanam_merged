...
import { SBtn, SCard, SAlert, SBadge, SLoader, SEmpty } from '../../components/ui'
import {
...
export default function CollegeAdvisorDashboardPage() {
  ...
  const handleOpenCareerDetails = async (slug) => {
    ...
    try {
      const res = await axiosInstance.get(`/college-advisor/career/${slug}`)
      if (res.data?.success) {
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          {filteredRecommendations.length === 0 ? (
            <SEmpty icon="🎯" title="No recommendations yet" desc="We couldn't match any careers for your current profile. Update your academic profile or target career and try again." />
          ) : (
