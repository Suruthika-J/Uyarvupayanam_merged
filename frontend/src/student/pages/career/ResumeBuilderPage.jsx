...
export default function ResumeBuilderPage() {
  ...
  const fetchResume = async () => {
    ...
    setError('')
    try {
      const res = await axiosInstance.get('/study-tools/resume-builder')
      if (res.data?.success && res.data.resumeData) {
        ...
        setError('The AI couldn’t assemble a resume from your profile yet.')
      }
    } catch (err) {
      console.warn('Failed to load resume builder data')
      setError('We couldn’t reach the resume builder. Please try again in a moment.')
    } finally {
      setLoading(false)
    }
  }
  ...
  if (loading) {
    return (
      <div style={{ maxWidth: 900, margin: '0 auto', paddingTop: 20 }}>
        <AIGenerating
          label="Assembling your professional resume"
          sub="Auto-populating your header, projects, competencies, and certifications from your profile telemetry..."
        />
      </div>
    )
  }
  ...
  if (error) {
    return (
      <div style={{ maxWidth: 900, margin: '0 auto', paddingTop: 20 }}>
        <AIFailure
          title="Couldn’t build your resume"
          message={error}
          onRetry={fetchResume}
          retryLabel="Retry Resume Builder"
        />
      </div>
    )
  }
    ...
    return (
      <div style={{ maxWidth: 900, margin: '0 auto', paddingTop: 20 }}>
        <SEmpty
          ...
          desc="Complete your academic profile (degree, CGPA, skills, projects) so the AI can draft a professional resume for you."
        />
      </div>
    )
  }
