...
import { courseService, careerService, collegeService } from '../../services'
import {
  ...
  FiShield, FiHelpCircle, FiChevronRight, FiFileText, FiUsers
} from 'react-icons/fi'
...
const USER_TYPES = [
  {
    ...
    audienceTag: 'Best for ages 10–17',
    accentColor: '#6d28d9',
    accentBg: '#ede9fe',
    accentSoft: '#f5f3ff',
    icon: FiCompass,
    countKey: 'careers',
    countLabel: 'career paths to explore',
    countFallback: '200+',
    image: 'https://images.unsplash.com/photo-1503676260728-1c00da094a0b?auto=format&fit=crop&w=800&q=80',
    desc: 'Discover your ideal stream early and never miss a scholarship or entrance deadline.',
    points: [
      { lead: 'Class 5, 8, 10, 12', text: 'Milestone Guidance' },
      { lead: 'Gamified', text: 'Logic & Communication Activities' },
      { lead: 'TNEA', text: 'Engineering Cutoffs & Quota Data' },
      { lead: 'Alerts', text: 'for Entrance Exams & Scholarships' }
    ],
  ...
  },
  {
    ...
    audienceTag: 'Best for undergrads',
    accentColor: '#047857',
    accentBg: '#d1fae5',
    accentSoft: '#ecfdf5',
    icon: FiBookOpen,
    countKey: 'courses',
    countLabel: 'courses & degrees mapped',
    countFallback: '500+',
    image: 'https://images.unsplash.com/photo-1523240795612-9a054b0db644?auto=format&fit=crop&w=800&q=80',
    desc: 'Pick the degree and specialization that actually aligns with your career goals.',
    points: [
      { lead: 'Courses & Mappings', text: 'across 500+ verified colleges' },
      { lead: 'Branches', text: 'Deep-dives into specializations' },
      { lead: 'Salaries', text: 'real-world scopes & trends' },
      { lead: 'Advisor', text: 'AI guidance & mentor requests' }
    ],
  ...
  },
  {
    ...
    audienceTag: 'Best for final-year & alumni',
    accentColor: '#1e40af',
    accentBg: '#dbeafe',
    accentSoft: '#eff6ff',
    icon: FiBriefcase,
    countKey: 'colleges',
    countLabel: 'colleges & post-grad options',
    countFallback: '500+',
    image: 'https://images.unsplash.com/photo-1522071820081-009f0129c71c?auto=format&fit=crop&w=800&q=80',
    desc: 'Chart your next move — post-grad, competitive exams, or a confident career pivot.',
    points: [
      { lead: 'Post-Grad', text: '& certification discovery' },
      { lead: 'Exams', text: 'GATE, CAT, TNPSC readiness' },
      { lead: 'Transitions', text: 'to new professional careers' },
      { lead: '1-on-1', text: 'Academic Advisor support' }
    ],
...
// ── User Type (Audience) Card Component ──────────────────────────────────────
function UserTypeCard({ user, index, count }) {
  const [hovered, setHovered] = useState(false)
  const IconComp = user.icon

  const countValue = count != null && count > 0 ? count : null
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: 22,
        overflow: 'hidden',
        border: hovered ? `1.5px solid ${user.accentColor}` : '1px solid var(--s-border)',
        boxShadow: hovered
          ? `0 24px 50px ${user.accentColor}22, 0 8px 18px rgba(0,0,0,0.08)`
          : 'var(--s-shadow)',
    ...
    >
      {/* Top Photo Banner */}
      <div style={{ position: 'relative', height: 150, width: '100%', overflow: 'hidden', background: '#0b1329', flexShrink: 0 }}>
        <img
          src={user.image}
          alt={user.title}
          loading="lazy"
        ...
        <div style={{
          position: 'absolute',
          inset: 0,
          background: `linear-gradient(180deg, ${user.accentColor}00 30%, ${user.accentColor}40 100%)`
        }} />
        ...
        <div style={{
          ...
          <FiUsers size={13} style={{ color: '#fff' }} /> {user.audienceTag}
        </div>
      </div>
      ...
      <div style={{
        ...
        background: `linear-gradient(180deg, ${user.accentSoft} 0%, #ffffff 55%)`
      }}>
        <h3 style={{ fontSize: 22, fontWeight: 900, color: 'var(--s-text)', margin: '0 0 4px', fontFamily: 'var(--s-font-display)' }}>
          {user.title}
        </h3>
        <div style={{ fontSize: 12.5, fontWeight: 800, color: user.accentColor, marginBottom: 12, letterSpacing: '0.02em' }}>
          {user.subtitle}
        </div>
        ...
        <div style={{
          ...
          <div>
            <div style={{ fontSize: 17, fontWeight: 900, color: user.accentColor, lineHeight: 1.1 }}>
              {countValue != null ? countValue.toLocaleString('en-IN') + '+' : user.countFallback}
            </div>
            <div style={{ fontSize: 11.5, color: 'var(--s-text3)', fontWeight: 600 }}>
              {user.countLabel}
            </div>
          </div>
        </div>
        ...
        {/* Scannable Bullets with bolded lead */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 9, marginBottom: 22, flex: 1 }}>
          {user.points.map((pt, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, fontSize: 12.5, color: 'var(--s-text2)', lineHeight: 1.5 }}>
              <FiCheckCircle size={15} style={{ color: user.accentColor, flexShrink: 0, marginTop: 2 }} />
              <span>
                <strong style={{ color: 'var(--s-text)', fontWeight: 800 }}>{pt.lead}: </strong>
                {pt.text}
              </span>
            </div>
          ))}
        ...
        <Link
          ...
          style={{
            ...
            padding: '12px 18px',
            borderRadius: 12,
            background: hovered ? user.accentColor : user.accentBg,
            color: hovered ? '#ffffff' : user.accentColor,
            border: `1.5px solid ${hovered ? user.accentColor : 'transparent'}`,
            fontWeight: 800,
            fontSize: 14,
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'space-between',
            boxShadow: hovered ? `0 8px 22px ${user.accentColor}44` : 'none',
            transition: 'all 0.25s ease'
          }}
        >
...
export default function CommonPublicLandingPage() {
  ...
  const [liveCounts, setLiveCounts] = useState({ careers: null, courses: null, colleges: null })
  ...
  useEffect(() => {
    courseService.getAll()
      .then(res => {
        ...
        const count = (res && Array.isArray(res) ? res.length : (res && res.data && res.data.length)) || null
        setLiveCounts(prev => ({ ...prev, courses: count }))
      })
    ...
    careerService.getAll()
      .then(res => {
        const count = (res && res.count) || (res && Array.isArray(res) ? res.length : null) || null
        setLiveCounts(prev => ({ ...prev, careers: count }))
      })
      .catch(() => {})
    ...
    collegeService.getAll()
      .then(res => {
        const count = (res && res.count) || (res && Array.isArray(res) ? res.length : null) || null
        setLiveCounts(prev => ({ ...prev, colleges: count }))
      })
      .catch(() => {})
  }, [])
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 28 }}>
              {USER_TYPES.map((user, idx) => (
                <UserTypeCard
                  ...
                  user={user}
                  index={idx}
                  count={liveCounts[user.countKey]}
                />
              ))}
            </div>
          </div>
