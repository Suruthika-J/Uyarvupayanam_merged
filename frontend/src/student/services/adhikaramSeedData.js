// Offline/demo fallback mirror of the Adhikaram Cartoon Theme System.
// Kept in sync with backend/seeders/seedAdhikarams.js - the backend is the
// single source of truth; this data only powers browse/demo mode when the
// API is unreachable. Content identical: 6 chapters x 10 kurals = 60 verses.
...
export function findSeedAdhikaram(idVal) {
  const sorted = [...seedAdhikarams].sort((a, b) => a.number - b.number)
  const idx = sorted.findIndex((a) => String(a.id) === String(idVal))
  if (idx === -1) return null
  const rec = sorted[idx]
  return {
    ...rec,
    navigation: {
      previousId: idx > 0 ? sorted[idx - 1].id : null,
