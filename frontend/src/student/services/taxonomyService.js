...
export const taxonomyService = {
  ...
  getFields: async () => {
    try {
      const res = await axiosInstance.get('/taxonomy/fields', { timeout: 3000 })
  ...
  getDegrees: async (fieldId = 'engineering') => {
    try {
      const res = await axiosInstance.get('/taxonomy/degrees', { params: { fieldId }, timeout: 3000 })
  ...
  getDomains: async (fieldId = 'engineering') => {
    try {
      const res = await axiosInstance.get('/taxonomy/domains', { params: { fieldId }, timeout: 3000 })
  ...
  getSpecializations: async (fieldId = 'engineering', domainName = '') => {
    try {
      const res = await axiosInstance.get('/taxonomy/specializations', { params: { fieldId, domainName }, timeout: 3000 })
  ...
  getCertifications: async (fieldId = 'engineering') => {
    try {
      const res = await axiosInstance.get('/taxonomy/certifications', { params: { fieldId }, timeout: 3000 })
