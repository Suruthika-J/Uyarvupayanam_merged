...
const SOURCE_FILE = 'Ayurveda_Colleges_Course.xlsx';
...
const importAyurvedaExcel = async (forceSync = false) => {
  const startTime = Date.now();
  console.log(`[Ayurveda Import] Starting Ayurveda Medical College-Course Mapping import...`);

  const excelPath = path.join(__dirname, '../uploads/Ayurveda_Colleges_Course.xlsx');
  if (!fs.existsSync(excelPath)) {
    console.error(`[Ayurveda Import] Excel file not found at: ${excelPath}`);
    return { success: false, error: `Excel file not found at: ${excelPath}` };
  }

  const stateFilePath = path.join(__dirname, '../uploads/.ayurveda_excel_state.json');
  ...
  if (!forceSync && fs.existsSync(stateFilePath)) {
    try {
      ...
      if (savedState.mtimeMs === currentMtime && savedState.size === currentSize) {
        console.log(`[Ayurveda Import] Excel file unchanged. Skipping.`);
        return { success: true, skipped: true, message: 'Excel file unchanged.' };
      }
    } catch (err) {
      console.warn(`[Ayurveda Import] Failed to read state file. Forcing run.`);
    }
  }

  console.log(`[Ayurveda Import] Reading Ayurveda_Colleges_Course.xlsx...`);
  ...
  console.log(`[Ayurveda Import] Parsed ${rows.length} college rows from Excel.`);
  ...
  try {
    ...
    const batchId = `AYURVEDA-EXCEL-${Date.now()}`;

    const csvCollegeCourseMap = new Map();

    console.log(`[Ayurveda Import] Updating existing colleges...`);
    ...
    for (const row of rows) {
      ...
      if (!college) {
        ...
        college = {
          ...
          state: 'Tamil Nadu',
          collegeType: 'Ayurveda',
          type: 'Ayurveda Medical College'
        };
    ...
    console.log(`[Ayurveda Import] Creating new colleges...`);

    if (collegesToInsert.length > 0) {
      await College.insertMany(collegesToInsert);
      console.log(`[Ayurveda Import] Inserted ${collegesToInsert.length} new colleges.`);
    }
    if (collegeBulkOps.length > 0) {
      await College.bulkWrite(collegeBulkOps);
      console.log(`[Ayurveda Import] Updated ${collegeBulkOps.length} existing colleges.`);
    }

    console.log(`[Ayurveda Import] Processing courses...`);
    ...
    for (const courseName of courseNamesToCreate) {
      ...
      coursesToInsert.push({
        ...
        shortDescription: `Bachelor of Ayurvedic Medicine and Surgery (${courseName})`,
        isImported: true,
        status: 'active',
        isPublished: true,
        source: SOURCE_FILE
    ...
    if (coursesToInsert.length > 0) {
      ...
      console.log(`[Ayurveda Import] Created ${inserted.length} new courses.`);
      for (const c of inserted) {
        courseMapByNormName.set(c.courseName.toLowerCase().trim(), c);
      }
    }

    console.log(`[Ayurveda Import] Updating College-Course Mapping...`);
    ...
    console.log(`[Ayurveda Import] Processing ${mappingOps.length} mapping operations...`);
    ...
    console.log(`\n[Ayurveda Import] Sync complete in ${report.timeTakenMs}ms:`);
    ...
    console.error(`[Ayurveda Import] Import failed:`, err);
    return { success: false, error: err.message };
  }
};

module.exports = { importAyurvedaExcel };
